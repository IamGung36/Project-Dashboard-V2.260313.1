...
class DashboardApp {
  ...
  init() {
    ...
    // Setup Google Sheet Sync Event Handlers
    ...
    const triggerSync = () => this.syncFromGoogleSheets();

    if (sidebarSyncBtn) sidebarSyncBtn.addEventListener('click', triggerSync);
    if (mobileSyncBtn) mobileSyncBtn.addEventListener('click', triggerSync);
    if (settingsSyncBtn) settingsSyncBtn.addEventListener('click', triggerSync);

    if (settingsDisconnectBtn) {
      settingsDisconnectBtn.addEventListener('click', () => {
        if (confirm('คุณต้องการตัดการเชื่อมต่อกับ Google Sheet หรือไม่? (ระบบจะเปลี่ยนไปรันโหมด Local Storage แทน)')) {
          window.db.setGasUrl('');
          const gasStatusDiv = document.getElementById('gas-connection-status');
          if (gasStatusDiv) {
            gasStatusDiv.className = 'mt-3 fs-7 text-success';
          ...
          const apiUrlInput = document.getElementById('gas-api-url');
  ...
  setTheme(theme) {
    ...
    // Update Leaflet tile layer if maps are loaded
    const tileUrl = theme === 'dark' 
      ? 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png'
      : 'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png';
      
    if (this.portfolioMap) {
      this.portfolioMap.eachLayer(layer => {
    ...
    if (this.overviewMap) {
      this.overviewMap.eachLayer(layer => {
        if (layer instanceof L.TileLayer) {
          layer.setUrl(tileUrl);
        }
      });
  ...
  switchPage(pageId) {
    ...
    if (pageId === 'portfolio') {
      ...
      if (this.portfolioMap) {
        this.portfolioMap.invalidateSize();
        this.portfolioMap.setView([this.thailandCenter.lat, this.thailandCenter.lng], 6);
      }
    } else if (pageId === 'overview') {
      this.renderOverviewCharts();
      if (this.overviewMap) {
        this.overviewMap.invalidateSize();
        this.overviewMap.setView([this.thailandCenter.lat, this.thailandCenter.lng], 6);
      }
  ...
  updateViews() {
    ...
    this.renderSettingsGasUrl();
    this.updateMapMarkers();
    this.updateSyncWidgets();
  }

  updateSyncWidgets() {
    const gasUrl = window.db.getGasUrl();
    const isSaving = window.db.isSaving;
    const lastSaveTime = window.db.lastSaveTime;

    const sidebarDbStatus = document.getElementById('sidebar-db-status');
    const sidebarSyncTime = document.getElementById('sidebar-sync-time');
    const sidebarSyncBtn = document.getElementById('sidebar-sync-btn');

    const mobileDbStatus = document.getElementById('mobile-db-status');
    const mobileSyncBtn = document.getElementById('mobile-sync-btn');
    ...
    let syncTimeText = 'Not synced';
    if (lastSaveTime) {
      const date = new Date(lastSaveTime);
      const hours = String(date.getHours()).padStart(2, '0');
      const minutes = String(date.getMinutes()).padStart(2, '0');
      const seconds = String(date.getSeconds()).padStart(2, '0');
      syncTimeText = `Synced at ${hours}:${minutes}:${seconds}`;
    }

    if (gasUrl) {
      // Show Google Sheets Connection indicators
      if (sidebarDbStatus) {
        sidebarDbStatus.className = 'badge bg-success px-2 py-1';
        sidebarDbStatus.innerHTML = '<i class="fas fa-link me-1"></i>Connected';
        sidebarDbStatus.style.backgroundColor = 'var(--primary-color)';
      }
      if (sidebarSyncTime) {
        sidebarSyncTime.innerText = isSaving ? 'Saving to sheet...' : syncTimeText;
      }
      if (sidebarSyncBtn) {
        sidebarSyncBtn.style.display = 'block';
        const icon = sidebarSyncBtn.querySelector('i');
        if (icon) {
          if (isSaving) icon.classList.add('fa-spin');
          else icon.classList.remove('fa-spin');
        }
      }

      if (mobileDbStatus) {
        mobileDbStatus.className = 'badge bg-success me-2';
        mobileDbStatus.innerHTML = 'Connected';
      }
      if (mobileSyncBtn) {
        mobileSyncBtn.style.display = 'block';
        const icon = mobileSyncBtn.querySelector('i');
        if (icon) {
          if (isSaving) icon.classList.add('fa-spin');
          else icon.classList.remove('fa-spin');
        }
      ...
      if (settingsSyncPanel) {
        settingsSyncPanel.style.setProperty('display', 'flex', 'important');
      }
      if (settingsSyncTime) {
        settingsSyncTime.innerText = isSaving ? 'Saving changes to Google Sheets in real-time...' : syncTimeText;
      }
    } else {
      // Local Storage Mode
      if (sidebarDbStatus) {
        sidebarDbStatus.className = 'badge bg-secondary px-2 py-1';
        sidebarDbStatus.innerText = 'Local';
      }
      if (sidebarSyncTime) {
        sidebarSyncTime.innerText = 'Offline Mode';
      }
      if (sidebarSyncBtn) {
        sidebarSyncBtn.style.display = 'none';
      }

      if (mobileDbStatus) {
        mobileDbStatus.className = 'badge bg-secondary me-2';
        mobileDbStatus.innerText = 'Local';
      }
      if (mobileSyncBtn) {
        mobileSyncBtn.style.display = 'none';
      }

      if (settingsSyncPanel) {
        settingsSyncPanel.style.setProperty('display', 'none', 'important');
      }
    }
  }
  ...
  syncFromGoogleSheets() {
    const gasUrl = window.db.getGasUrl();
    if (!gasUrl) {
      alert('ไม่ได้เชื่อมต่อกับ Google Sheet กรุณาใส่ Web App URL ในหน้าตั้งค่าก่อน');
      return;
    }

    // Add spinner to all sync buttons
    const btns = [
      document.getElementById('sidebar-sync-btn'),
      document.getElementById('mobile-sync-btn'),
      document.getElementById('settings-sync-btn')
    ];
    ...
    console.log('Force-syncing database from Google Sheets...');
    
    return window.db.fetchFromServer(gasUrl, true)
      .then(serverDb => {
        console.log('Force-sync complete: views updated');
        ...
        // Show success connection status on settings page if visible
        const gasStatusDiv = document.getElementById('gas-connection-status');
        if (gasStatusDiv && window.db.getGasUrl()) {
          gasStatusDiv.className = 'mt-3 fs-7 text-success';
          gasStatusDiv.innerHTML = '<i class="fas fa-check-circle me-2"></i>ดึงข้อมูลล่าสุดจาก Google Sheets เรียบร้อยแล้ว!';
          gasStatusDiv.style.display = 'block';
        }
      })
      .catch(err => {
      ...
      .finally(() => {
        ...
        window.db.lastSaveTime = Date.now(); // update timestamp
        this.updateSyncWidgets();
      });
  }

  initMaps() {
    const thailandCenter = [this.thailandCenter.lat, this.thailandCenter.lng];
    ...
    if (portMapEl) {
      if (this.portfolioMap) {
        try { this.portfolioMap.remove(); } catch(e) {}
      }
      this.portfolioMap = L.map('portfolio-map', {
        center: thailandCenter,
        zoom: 6,
        attributionControl: true
      });
      L.tileLayer(tileUrl, { attribution: tileAttribution, maxZoom: 20 }).addTo(this.portfolioMap);
      
      // Compatibility helper methods for search/resizing
      this.portfolioMap.setCenter = (coords) => {
        this.portfolioMap.setView([coords.lat, coords.lng], this.portfolioMap.getZoom());
      };
      this.portfolioMap.setZoom = (zoom) => {
        this.portfolioMap.setView(this.portfolioMap.getCenter(), zoom);
      };
    }
    ...
    if (overMapEl) {
      ...
      L.tileLayer(tileUrl, { attribution: tileAttribution, maxZoom: 20 }).addTo(this.overviewMap);
      
      // Compatibility helper methods for search/resizing
      this.overviewMap.setCenter = (coords) => {
        this.overviewMap.setView([coords.lat, coords.lng], this.overviewMap.getZoom());
      };
      this.overviewMap.setZoom = (zoom) => {
        this.overviewMap.setView(this.overviewMap.getCenter(), zoom);
      };
    }
  ...
  updateMapMarkers() {
    ...
    this.portfolioMarkers.forEach(m => m.remove());
    this.overviewMarkers.forEach(m => m.remove());
    this.portfolioMarkers = [];
    this.overviewMarkers = [];
    ...
    projects.forEach(p => {
      ...
      const markerIcon = L.icon({
        iconUrl: this.getPinIconSvg(pinColor),
        iconSize: [36, 36],
        iconAnchor: [18, 36],
        popupAnchor: [0, -36]
      });
      ...
      const marker = L.marker([p.lat, p.lng], { icon: markerIcon, title: p.name });

      // Bind popup with styling
  ...
  placeSearchMarker(map, coords, title) {
    ...
    this[markerKey] = L.marker([coords.lat, coords.lng], { icon: markerIcon, title: title }).addTo(map);

    const popupContent = `<div class="map-info-window" style="color: #1a251f; padding: 6px; max-width: 260px;">
    ...
    this[markerKey].bindPopup(popupContent).openPopup();
  }
