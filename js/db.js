...
const DB_KEY = 'PROJECT_DASHBOARD_DB_V4'; // Version 4 to force clear browser cache and reload from Google Sheet
...
class LocalDatabase {
  ...
  getGasUrl() {
    let url = localStorage.getItem('PROJECT_DASHBOARD_GAS_URL');
    if (url === null) {
      const defaultUrl = 'https://script.google.com/macros/s/AKfycbzaeR3Q4NloXjbkQHe2Nv-7AwtzZi7EZ9FTaxK_vOorNS0dhWh_2sOiX97J38zJHi0EUA/exec';
      localStorage.setItem('PROJECT_DASHBOARD_GAS_URL', defaultUrl);
      return defaultUrl;
    }
    return url;
  }
  ...
  setGasUrl(url) {
    if (url) {
      ...
      localStorage.setItem('PROJECT_DASHBOARD_GAS_URL', '');
    }
  }
  ...
  startRealtimeSync() {
    setInterval(() => {
      ...
      if (this.lastSaveTime && (Date.now() - this.lastSaveTime < 10000)) {
  ...
  patchAndMigrate() {
    ...
    const migrateProjectFields = (p) => {
      ...
      // 4. Normalize lat and lng to either numbers or null
      if (p.hasOwnProperty('lat')) {
        if (p.lat === null || p.lat === undefined || p.lat === '' || isNaN(Number(p.lat)) || String(p.lat).trim() === '') {
          if (p.lat !== null) {
            p.lat = null;
            migrated = true;
          }
        } else {
          const parsed = parseFloat(p.lat);
          if (p.lat !== parsed) {
            p.lat = parsed;
            migrated = true;
          }
      ...
      if (p.hasOwnProperty('lng')) {
        if (p.lng === null || p.lng === undefined || p.lng === '' || isNaN(Number(p.lng)) || String(p.lng).trim() === '') {
          if (p.lng !== null) {
            p.lng = null;
            migrated = true;
          }
        } else {
          const parsed = parseFloat(p.lng);
          if (p.lng !== parsed) {
            p.lng = parsed;
            migrated = true;
          }
